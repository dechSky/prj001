...
웹 분석 + 코드 리뷰 기준: winit 창 요청값은 logical size로 두고, `WindowEvent::Resized`/wgpu surface는 physical pixel size를 그대로 쓴다. cosmic-text `Metrics`도 physical pixel 기준이므로 `FONT_SIZE(14.0 logical) * window.scale_factor()`를 renderer에 전달한다. `ScaleFactorChanged`에서는 atlas/font metrics를 재생성하고 pane viewport/PTY size를 재계산한다. monitor 해상도 bucket 방식은 모니터별 체감 크기 불일치로 폐기.

## 2026-05-13 갱신 — M15 첫 slice: Cmd+R respawn

코드 리뷰 기준, `AgentKind`/preset 위치 결정 전에는 active session respawn이 M15에서 가장 작은 구현 단위다. `Cmd+R`은 active pane의 기존 `Session.command`를 그대로 재사용해 새 PTY를 spawn하고, 성공 후 pane의 `SessionId`만 교체한다. scrollback은 초기화되고 기존 session은 code `-1` lifecycle exit로 기록한다. `Cmd+Shift+N` quick spawn과 preset metadata는 `PaneMeta`/`SessionMeta` 위치 결정 후 후속 slice로 진행.

## 2026-05-13 갱신 — M15 quick spawn boundary

`Cmd+Shift+N` quick spawn은 `Session.kind`/`AgentKind`를 core에 추가하지 않고 `Config.quick_spawn_presets: Vec<(key, SessionSpec)>` 형태의 데이터 주입으로 처리한다. 일반 실행 기본값은 `s=shell`만 제공하고, 추가 preset은 외부 binary가 `Config`로 주입한다. core status bar는 계속 title 기반으로 표시한다.
...
**완료 상태**: M1~M10 + M17(reflow) + **M11 workspace 분리**(commit `396a536`, cargo test 66 → 75) + **M12 Session 추출** + **M13 BSP layout tree**(commits `d716b70`~`ed52803`, 후속 redraw fix `b351f69`~`248a195`) 완료. 2026-05-12 `cargo test` 104개 통과.

**상용 급 OSS 지향 확장**: 학습 목적에서 "macOS 전용 빠르고 정확한 GPU 터미널, Alacritty/Ghostty 하위 80% 사용 사례 통과, AI CLI 깨뜨리지 않는 host terminal"로 1차 목표 확장. 광역 웹 분석(Warp/iTerm2/Ghostty/WezTerm/Alacritty/Kitty/Terminal.app) + 2nd opinion 리뷰 완료.
...
본 로드맵 §2~§10의 M11~M19 모두 새 trajectory와 충돌 또는 미정렬. **m12-m16 plan + 상용 갭 분석 P0~P3 권고**가 정본이며, 본 로드맵 잔여 항목은 다음 분류로 재처리 예정:
...
6. **환경설정** (원래 M19): 상용 갭 분석에서 config file + keybindings + profiles는 P1로 격상. 세부 정책은 `docs/settings-and-context-menu-plan.md`.

### 의도적 미지원 재검토

- **OSC 52** 미지원 → **opt-in (기본 off)**: `allow_osc52 = off/ask/local-only/on`. CLI `/copy` 호환.
- **Kitty graphics** 미지원 → **P3 experimental flag**. 인라인 이미지 트렌드 대비.
...
- **P1**: OSC 8/133/7 + title stack + config + keybindings + profiles + context menu/right-click policy + KKP + mouse reporting + find + buffer clear + font zoom
...
| **M19-2 right-click action enum** | 현 hard-coded 우클릭 동작을 `copy/paste/copy_or_paste/context_menu/ignore` enum으로 이관 |
| **M19-3 context menu overlay MVP** | Copy/Paste/Clear/New Split/New Tab/Close Pane. 우클릭/secondary click/Ctrl-click 정규화 |
| **M19-4 default right-click switch** | context menu MVP 뒤 기본값을 `context_menu`로 전환 |
| **M19-5 mouse/key binding** | physical key 기본, `Cmd+Shift+K` 같은 global shortcut 충돌 대응 |
| **M19-6 profile abstraction** | Terminal.app처럼 Profile = font/color/shell/window behavior 묶음. GUI 전 internal struct |
| **M19-7 hot-reload** | `notify` 크레이트로 watcher. atlas 재생성 / 색 갱신 |
| **M19-8 Cmd+, settings GUI** (선택) | 별도 창 또는 in-app overlay. 1차 cut: 파일 편집만 |
...
4. **환각 방지 4원칙** — spec 인용 시 출처 표기, 진입 시 docs.rs/spec 재확인.
